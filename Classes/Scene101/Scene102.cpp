...
bool Scene102::init()
{
	...
	_label = Label::createWithTTF("Scene 102", "fonts/Marker Felt.ttf", 32);
	_label->setAlignment(cocos2d::TextHAlignment::CENTER); // 預設靠左對齊
	_label->setWidth(100);	// 設定每行文字的顯示寬度
	size = _label->getContentSize();
	_label->setPosition(Vec2(origin.x + visibleSize.width - size.width / 2 - 10, origin.y + visibleSize.height - size.height / 2 - 10));
	...
	//一般(非中文字)文字的顯示方式
	auto label1 = Label::createWithBMFont("fonts/couriernew32.fnt", "Scene 102");
